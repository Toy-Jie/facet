...
class SAMPNetScorer:
    ...
    def _load_model(self):
        ...
        try:
            ...
            if missing:
                logger.debug("[SAMP-Net] Note: %s model keys not in checkpoint", len(missing))
            if unexpected:
                logger.debug("[SAMP-Net] Note: %s checkpoint keys not in model", len(unexpected))
...
def create_samp_scorer(config) -> SAMPNetScorer:
    ...
    try:
        ...
        logger.error("Failed to initialize SAMP-Net scorer: %s", e)
        return None
